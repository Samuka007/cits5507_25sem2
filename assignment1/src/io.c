...
// Read matrix from file into flattened format
int read_matrix_from_file_flatten(const char *filename, float **matrix, int *rows, int *cols) {
    ...
    *matrix = allocate_matrix_flatten(*rows, *cols);
...
int write_matrix_to_file_flatten(const char *filename, float *matrix, int rows, int cols) {
    ...
    for (int i = 0; i < rows; i++) {
        for (int j = 0; j < cols; j++) {
            fprintf(file, "%.3f", matrix[i * cols + j]);
            if (j < cols - 1) {
                fprintf(file, " ");
            }
        }
...
// Print flattened matrix to stdout
...
// Generate random matrix in flattened format
float *generate_random_matrix_flatten(int rows, int cols, float min_val, float max_val) {
    float *matrix = allocate_matrix_flatten(rows, cols);
    ...
    #pragma omp parallel
    {
        ...
        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < cols; j++) {
                // Generate random float between min_val and max_val using fast xorshift
                unsigned int rand_int = xorshift32(&seed);
                matrix[i * cols + j] = ((float)rand_int * scale) + min_val;
            }
        }
    }
...
// Read matrix from file directly into padded flattened format
int read_matrix_into_padded_flatten(const char *filename, int kernel_height, int kernel_width,
    ...
    *padded_width = *original_width + pad_left + pad_right;
    ...
    // Allocate padded flattened matrix
    *padded = allocate_matrix_flatten(*padded_height, *padded_width);
    initialize_matrix_flatten(*padded, *padded_height, *padded_width, 0.0f);

    // Read matrix data directly into the center of the padded matrix
    for (int i = 0; i < *original_height; i++) {
        for (int j = 0; j < *original_width; j++) {
            if (fscanf(file, "%f", &(*padded)[(i + pad_top) * (*padded_width) + (j + pad_left)]) != 1) {
                perror("Error: Cannot read matrix element");
...
// Generate random matrix directly into padded flattened format
float *generate_random_matrix_into_padded_flatten(int height, int width, int kernel_height, int kernel_width,
    ...
    *padded = allocate_matrix_flatten(*padded_height, *padded_width);
    ...
    #pragma omp parallel
    {
        ...
        for (int i = 0; i < height; i++) {
            for (int j = 0; j < width; j++) {
                // Generate random float between min_val and max_val using fast xorshift
                unsigned int rand_int = xorshift32(&seed);
                (*padded)[(i + pad_top) * (*padded_width) + (j + pad_left)] = ((float)rand_int * scale) + min_val;
            }
        }
    }
    ...
    // Return a pointer to the original data area (for compatibility with existing code)
    // This allows the caller to access the original matrix if needed
    return (*padded) + pad_top * (*padded_width) + pad_left;
}
